"""AI 自动踢动 15cm 红色正方体木块并向前行走。

木块通过读取 t800.xml 字符串后注入 <worldbody>，不会修改 t800.xml 文件。
策略给固定前向速度命令 [0.4, 0, 0]，由 MNN 输出关节动作，机器人会推动木块前进。

macOS 下请用 mjpython 运行：
    mjpython 23_t800_mnn_kick_cube.py
"""
...
# 木块放在机器人前上方地面上，机器人前向行走时脚会先踢到它。
CUBE_POS = [0.5, 0.0, 0.075]
CUBE_HALF_SIZE = 0.075
CUBE_MASS = 0.5

FORWARD_COMMAND = np.array([0.4, 0.0, 0.0])
...
def main():
    ...
    base_id = mujoco.mj_name2id(
        model, mujoco.mjtObj.mjOBJ_BODY, "LINK_BASE"
    )
    ...
    print_interval = int(0.5 / timestep)
    step = 0
    ...
    def one_step():
        ...
        if step % print_interval == 0:
            ...
            cube = data.xpos[cube_body_id]
            print(
                f"time={step * timestep:5.2f}s "
                f"base=({base[0]:7.3f}, {base[1]:7.3f}, {base[2]:7.3f}) "
                f"cube=({cube[0]:7.3f}, {cube[1]:7.3f}, {cube[2]:7.3f})"
            )

        step += 1

    print("AI 前向控制已启动，速度命令 [0.4, 0.0, 0.0]")
