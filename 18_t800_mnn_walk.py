"""使用 MNN 策略控制 t800 向前行走（最小教学版）。

观测布局：
    1083 = 72 维单步观测 * 15 步历史 + 3 维速度命令
    单步观测 = [关节角度偏移, 关节角速度, 上一动作, 身体角速度, 重力投影]
"""
...
DEFAULT_Q = np.array(
    [-0.06, 0.0, 0.0, 0.12, -0.06, 0.0] * 2  # 双腿
    + [0.0]                                   # 腰
    + [0.0, 0.15, 0.0, -0.25, 0.0]            # 左臂
    + [0.0, -0.15, 0.0, -0.25, 0.0]           # 右臂
    + [0.0, 0.0],                             # 头
)
...
# 观测缩放：dof_pos / dof_vel / prev_action / ang_vel / gravity
OBS_SCALE = np.array(
    [1.0] * 22 + [0.05] * 22 + [1.0] * 22 + [1.0] * 3 + [1.0] * 3
)

COMMAND = np.array([0.4, 0.0, 0.0])     # 前进 0.4 m/s
...
# 设置 True 后可在 MuJoCo viewer 中观看；macOS 需用 mjpython 运行
USE_VIEWER = True

# SDK 中注释给出的 floating_base_homing 初始姿态
...
def main():
    ...
    def one_step():
        ...
        # 底层 PD 每个 MuJoCo 步都执行
        apply_pd_control(model, data, controls, targets)

        if step % print_interval == 0:
            x = data.xpos[base_id, 0]
            z = data.xpos[base_id, 2]
            print(f"time={step * timestep:5.2f}s  base_x={x:7.3f}m  base_z={z:7.3f}m")
