"""WASD 控制 t800 行走，木块绑定在右手腕上跟随移动。

木块通过读取 t800.xml 字符串后注入 <worldbody>，不会修改 t800.xml 文件。
每个仿真步都会把木块的 free joint 同步到 LINK_WRIST_END_R 的位置和姿态。
...
macOS 下请用 mjpython 运行：
    mjpython 24_t800_wasd_wrist_cube.py
"""
...
# 15cm 正方体木块：MuJoCo box 的 size 是半边长，所以填 0.075m。
# CUBE_POS 只是 XML 里的初始占位，实际位置会在 mj_forward 后绑定到右手腕。
CUBE_POS = [0.2, 0.0, 1.2]
CUBE_HALF_SIZE = 0.075
CUBE_MASS = 0.5

WRIST_BODY_NAME = "LINK_WRIST_END_R"
# 木块中心相对右手腕 body 坐标系的偏移量，默认重合。
CUBE_WRIST_OFFSET = [0.0, 0.0, 0.0]
...
def attach_cube_to_wrist(model, data, cube_joint_id, wrist_id):
    """把木块 free joint 的位姿同步到右手腕 body。"""
    qpos_adr = model.jnt_qposadr[cube_joint_id]
    dof_adr = model.jnt_dofadr[cube_joint_id]

    wrist_pos = data.xpos[wrist_id] + CUBE_WRIST_OFFSET
    data.qpos[qpos_adr:qpos_adr + 3] = wrist_pos

    # 让木块姿态也跟随手腕姿态。
    wrist_mat = data.xmat[wrist_id]
    mujoco.mju_mat2Quat(
    ...
    # 手动改过 qpos/qvel 后，重新 forward 一次，确保 xpos/xmat 立刻刷新。
...
def main():
    ...
    # 初始化时立即把木块放到右手腕上。
    attach_cube_to_wrist(model, data, cube_joint_id, wrist_id)
    print(f"木块已绑定到 {WRIST_BODY_NAME}")
